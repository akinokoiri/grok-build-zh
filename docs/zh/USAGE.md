...
列表里 **人设 / 角色 / 代理 / 内置技能** 的 description 可套用中文（**ID 仍为英文**）：

```bash
# 在克隆的仓库根目录
bash docs/zh/bundled-zh/apply.sh
# 然后重启 grok-zh，打开 /skills 查看中文描述
```

市场插件、第三方技能的远程描述**不会**被此脚本改写（上游内容）。
...
| 人设 / 角色 / 代理 | ✅ | 界面 chrome + description（bundled-zh） |
| **内置技能 description** | ✅ | ID 英文；描述中文（`apply.sh` 写 SKILL.md） |
| Tips | ✅ | 剪贴板图片、规划提示、双击选词、SSH wrap、排队发送等 |
| `/docs` 用户指南 | ✅ | 应用内中文 |
| 市场常见插件 **description 映射** | ✅ | 运行时中文（ID 英文）；见 `bundled-zh/marketplace` |
| 未收录的市场/第三方描述 | ⚠️ | 回退上游英文 |
| 正规 i18n 切换 | 📋 | 规划中（当前硬编码中文） |

### 不会改成中文的

- 插件 / 技能 / 人设 **ID**（如 `reviewer`、`pptx`、`superpowers`）
- 模型名（`Grok 4.5`）、命令名（`/model`）、主题商品名
- 工具短名芯片（`Bash` / `Read` / `Edit` 等，与工具 ID 一致）
- 技能 **正文指令**（给模型的 SKILL.md 正文，默认英文）
- 会话标题（模型生成内容）
...
- 筛选可在 **全部 / 已启用 / 已禁用** 间循环
- 列表 **ID 英文**；内置技能描述用 `bundled-zh`；**市场常见插件**描述由 `grok-zh` 运行时映射为中文
- 市场可搜中文关键词（如「分支」「部署」）匹配已映射描述
- 未收录的第三方插件描述仍可能为英文
