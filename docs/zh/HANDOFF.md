...
项目已收敛为 Windows 11 x64 个人汉化发行版。已同步官方上游最新基线至 `72a61251`（包含 8 月 31 日与 9 月 1 日共 2 次大型 monorepo 同步），解决了更新劫持冲突；并已将活动会话界面与看板中的模式标记（`always-approve` 始终批准、`auto` 自动、`plan` 计划/计划审批/批注）及推理强度（如 `medium` 中等）完整接入集中语言包。

当前维护与提交轨迹：

- `bce3f556`：接入欢迎页菜单、公告、官方轮换提示、推理强度、批准模式和版本通道汉化；`v1.0.12-zh.2` 从该提交构建。
- `cb274bd9`：发布工作流优化：在跑窄测试前先行构建 release profile 产物。
- `c5ccdfdf`：同步官方上游 `official/main`（基线 `72a61251`，含安全补丁、Panel Dock、推理强度配置解耦及 MCP 多轮协商等 73 项更新），解决 Windows 更新劫持冲突。
- `8da8f347`：补全会话界面（`agent_view`）与看板（`dashboard`）中的模式标记及推理强度汉化，扩展集中语言包至 414 条。

`zh-CN` 是远端默认维护分支；`sync/official-2026-09-06` 是本轮同步分支。`legacy/full-fork-2026-08-29` 保留旧完整汉化分支，不要删除。
...
验证：翻译审计、格式检查、4 项共享语言包测试、2 项 protobuf 回归测试及 Windows pager-bin 编译检查均通过。面板定向测试仍被既有 PTY harness 的 Unix-only `process_has_exited_without_reap` 导入错误阻塞，未执行到面板测试；尚未进行新版 Release 构建和真实 TUI 冒烟。
...
- 审计脚本报告 299 个高置信英语候选。这是供 LLM 逐批审阅的队列，不表示应当机械地全部翻译；第三方、协议、测试和诊断文本必须继续排除。
