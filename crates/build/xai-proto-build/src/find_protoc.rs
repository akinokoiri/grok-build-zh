...
pub fn find_protoc() -> anyhow::Result<Option<PathBuf>> {
    ...
    if let Ok(protoc_env) = env::var("PROTOC") {
        ...
        // Windows action paths can be command shims rather than ordinary files.
        // Executability is the authoritative check; `try_exists` is only a fast path.
        if protoc.try_exists().unwrap_or(false) || check_protoc_good(&protoc).is_ok() {
