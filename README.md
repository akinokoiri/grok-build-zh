...
|------|------|
| 设置分类 / 设置项名称 | ✅ 已完成 |
| 设置项说明文字 | ✅ 大部分（含开/关/自动/滚轮/触控板等枚举显示） |
| 权限确认（是 / 始终允许 / 否…） | ✅ 核心路径 |
| 状态栏：正在回复、思考中、压缩中、排队… | ✅ |
| 回合事件：已工作、失败、压缩、重新登录… | ✅ |
| 快捷键面板 / 底栏 / 搜索占位 | ✅ 大部分 |
| 仪表盘标题、空态、固定区、筛选提示 | ✅ 大部分 |
| 欢迎页、模式切换横幅（含 CJK 宽度修复） | ✅ |
| 扩展 / 市场 / 技能 / MCP **界面** | ✅ 底栏、统计、徽章、筛选 |
| 代理 / 人设 **界面** | ✅ 标签「人设」、底栏、说明 |
| 用户指南（应用内 `/docs`） | ✅ 中文（部分长文精简） |
| 常见 Toast / 失败提示 / 相对时间 | ✅ 已覆盖一批 |
| 内置人设 description（可选覆盖包） | ✅ 见 [docs/zh/bundled-zh](docs/zh/bundled-zh/) |
| tips 长文案 | 🚧 核心已中文，可继续补 |
| 市场插件 / 第三方技能 **远程描述** | ⚠️ 上游英文；**ID 不汉化** |
| 可配置 en/zh 运行时切换（正规 i18n） | 📋 规划中（当前为硬编码中文分支） |

**原则：** 插件/技能/人设 **ID 保持英文**；**面向用户的描述** 在我们可控的范围内中文（bundled 可用覆盖包；远程市场描述需上游或本地映射）。

完整 100% 需社区持续 PR。欢迎对照英文截图提 Issue「漏网英文」。
