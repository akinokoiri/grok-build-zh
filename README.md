...
| 人设 / 角色 / 代理描述 | ✅ 可选覆盖 | 界面 chrome 已中文；description 见 [bundled-zh](docs/zh/bundled-zh/) |
| 常见 Tips（剪贴板/规划/选词/SSH/排队…） | ✅ 已完成 | 核心提示均中文 |
| 用户指南 `/docs` | ✅ 中文 | 部分长文精简 |
| 权限 / 状态栏 / 回合事件 / Toast | ✅ 核心路径 | |
| 市场插件 / 第三方技能 **远程描述** | ⚠️ 不可控 | 上游英文内容；**ID 不汉化** |
| 可配置 en/zh 运行时切换（正规 i18n） | 📋 规划中 | 当前为硬编码中文分支 |

**原则：** 插件 / 技能 / 人设 **ID 保持英文**；**面向用户的描述** 在可控范围内中文（内置可用 `bundled-zh`；远程市场需作者中文或本地映射）。
